...
import { useAgents, useBranches } from "@/hooks/useMasters";
...
export default function AgentsTab() {
  const { data, isLoading, upsert, remove, toggle } = useAgents();
  const { data: branches } = useBranches();
  const [open, setOpen] = useState(false);
  const [form, setForm] = useState<any>({});

  const openAdd = () => { setForm({ name: "", phone: "", branch_id: "", commission_rate: "" }); setOpen(true); };
  const openEdit = (row: any) => { setForm(row); setOpen(true); };

  const handleSave = async () => {
    const { id, name, phone, branch_id, commission_rate } = form;
    await upsert({ ...(id ? { id } : {}), name, phone, branch_id: branch_id || null, commission_rate: parseFloat(commission_rate || "0") });
    setOpen(false);
  };
  ...
  return (
    <div className="mt-4 space-y-4">
      <div className="flex justify-end">
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <Button onClick={openAdd} className="bg-accent text-accent-foreground hover:bg-accent/90"><Plus className="h-4 w-4 mr-1" />Add Agent</Button>
          </DialogTrigger>
          <DialogContent>
            <DialogHeader><DialogTitle>{form.id ? "Edit" : "Add"} Agent</DialogTitle></DialogHeader>
      ...
      <Table>
        <TableHeader><TableRow><TableHead>Name</TableHead><TableHead>Phone</TableHead><TableHead>Branch</TableHead><TableHead>Commission %</TableHead><TableHead>Status</TableHead><TableHead className="w-24">Actions</TableHead></TableRow></TableHeader>
        <TableBody>
          {(data || []).map((row: any) => (
            <TableRow key={row.id}>
              ...
              <TableCell>{Number(row.commission_rate).toFixed(2)}%</TableCell>
          ...
          {(!data || data.length === 0) && <TableRow><TableCell colSpan={6} className="text-center text-muted-foreground py-8">No agents yet</TableCell></TableRow>}
        </TableBody>
      </Table>
    </div>
  );
