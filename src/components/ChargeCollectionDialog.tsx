...
export default function ChargeCollectionDialog({
  ...
  // Fetch branch info for receipt header
  const { data: branch } = useQuery({
    queryKey: ["branch", profile?.branch_id],
  ...
  const collectMutation = useMutation({
    mutationFn: async () => {
      ...
      return { receiptNumber, paidLineItems, collectedAt: now };
    },
    onSuccess: ({ receiptNumber, paidLineItems, collectedAt }) => {
      ...
      setReceiptData({
        ...
        schemeName: loan.scheme?.name,
        collectedBy: profile?.full_name || user?.email || "—",
        paymentMode,
        reference: reference || undefined,
        totalAmount: finalAmount,
        collectedAt,
        lineItems: paidLineItems,
        branch: branch ? { name: branch.name, address: branch.address, phone: branch.phone } : undefined,
      });

      // Close collection dialog, open receipt
      onOpenChange(false);
      setReceiptOpen(true);
  ...
  return (
    <>
            ...
            <>
                  ...
                  <TableBody>
                    {unpaidRecords.map((rec: any) => {
                      ...
                      return (
                        <TableRow
                          key={rec.id}
                          className={isOverdue(rec) ? "bg-destructive/5" : rec.status === "pending" ? "bg-[hsl(var(--warning))]/5" : ""}
                        >
                          <TableCell>
                            <Checkbox
              ...
              <div className="space-y-3">
                ...
                <div className="bg-muted/50 rounded p-3 text-sm">
                  ...
                  <span className="font-mono font-medium">RCP-YYYYMMDD-###</span>
                  <span className="text-muted-foreground ml-2">(auto-generated on collection)</span>
                </div>
      ...
      {/* Receipt Modal - shown after successful collection */}
      <ReceiptModal
        open={receiptOpen}
